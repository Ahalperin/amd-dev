#!/bin/bash
# RCCL MPI Profiling Script with rocprofv3 + NPKit
# This script profiles RCCL multi-node MPI workloads with NPKit instrumentation
#
# IMPROVEMENTS:
...
# - Multi-layer profiling for complete picture:
...
# - Output: Perfetto traces (.pftrace) + NPKit traces (binary)
# - Shows complete 256M AllReduce performance:
#   GPU (kernels), Network (RCCL Comm), RCCL internals (NPKit)
# - View: Perfetto (https://ui.perfetto.dev) + chrome://tracing (NPKit)
...
echo "======================================================================"
echo "RCCL MPI Profiling with rocprofv3 + NPKit + DMA Tracking"
echo "======================================================================"
echo "Ranks:        $NP"
echo "Hosts:        $HOSTS"
...
# Run MPI profiling
echo "======================================================================"
echo "PROFILING: Running with rocprofv3 + NPKit instrumentation..."
echo "======================================================================"
echo "Configuration:"
echo "  - Profile ranks:       RANK 0 and RANK 1 with rocprofv3"
echo "  - NPKit:               ALL RANKS (fine-grained RCCL events)"
echo "  - Profilers:           rocprofv3 (GPU/API tracing) + NPKit (RCCL internals)"
...
echo "  - Output formats:      Perfetto (.pftrace) + NPKit (binary)"
...
echo "  ✓ NPKit: Fine-grained RCCL internals (send/recv primitives, channel ops)"
...
-x NPKIT_DUMP_DIR=${OUTPUT_DIR}/rank_\$OMPI_COMM_WORLD_RANK/npkit \
...
mkdir -p ${OUTPUT_DIR}/rank_\$OMPI_COMM_WORLD_RANK/npkit; \
...
# Check for NPKit data
NPKIT_COUNT=$(find ${OUTPUT_DIR} -path "*/npkit/*" -type f 2>/dev/null | wc -l)
echo "Found $NPKIT_COUNT NPKit trace files"
...
if [ $PROFILE_EXIT_CODE -eq 0 ] && [ "$PFTRACE_COUNT" -gt 0 ]; then
    ...
    echo "  - NPKit traces:      $NPKIT_COUNT files"
    ...
    echo ""
    echo "NPKit Analysis:"
    echo "   NPKit traces are in rank_*/npkit/ directories"
    echo "   - Use RCCL's npkit_trace_generator.py to convert to Chrome trace:"
    echo "     python3 npkit_trace_generator.py --npkit_dump_dir=rank_0/npkit --npkit_event_header_path=\$RCCL_HOME/src/include/npkit/npkit_event.h --output_dir=npkit_traces"
    echo "   - View the generated chrome_trace.json in chrome://tracing"
...
python3 $RCCL_HOME/tools/scripts/npkit_trace_generator.py \
  --npkit_dump_dir=/tmp/npkit/out \
  --npkit_event_header_path=$RCCL_HOME/src/include/npkit/npkit_event.h \
  --output_dir=/tmp/npkit/out