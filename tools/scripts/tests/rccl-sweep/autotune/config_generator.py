...
class TunerConfigGenerator:
    ...
    def _write_config(
        ...
        """Write tuner configuration file."""
        output_path = Path(output_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        
        with open(output_path, 'w') as f:
            # Write header
            f.write("# NCCL Tuner Configuration File (CSV Format)\n")
            f.write(f"# Generated by RCCL Auto-Tuner on {datetime.now().isoformat()}\n")
            f.write(f"# Source: {source_csv}\n")
            f.write("#\n")
            f.write("# Format: collective_type,min_bytes,max_bytes,algorithm,protocol,"
                    "channels,nNodes,nRanks,numPipeOps,regBuff\n")
            f.write("#\n")
            f.write("# Collective types: broadcast, reduce, allgather, reducescatter, allreduce\n")
            f.write("# Algorithms: tree, ring, collnet_direct, collnet_chain, nvls, nvls_tree, pat "
                    "or -1 to keep default\n")
            f.write("# Protocols: ll, ll128, simple or -1 to keep default\n")
            f.write("# Channels: number of channels to use, or -1 to keep default\n")
            f.write("#\n")
