...
usage() {
    ...
    echo "  -sv, --show-versions     Show existing package versions in given --from-user"
    ...
    echo ""
    echo "Examples:"
    echo "  # Deploy using defaults (version=develop, from_user=dn, to_user=dn)"
    echo "  $0"
    echo ""
    echo "  # Deploy a specific version"
    echo "  $0 -v my-feature-branch"
    echo ""
    echo "  # Deploy from user 'amir' to user 'testuser'"
    echo "  $0 -fu amir -tu testuser -v develop"
    echo ""
    echo "  # Show available package versions for user 'amir'"
    echo "  $0 -fu amir -sv"
    exit 0
}
